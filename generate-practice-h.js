...
async function generatePractice() {
    ...
    // load model
    const modelSource = "file://./Models/V3-RealData/model.json";
    ...
    function generateRawPractice(model, seedText, length) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            // Convert the input array to a 2D tensor
            const inputTensor2D = tf.tensor2d([input], [1, inputLength]);

            // Expand the dimensions of the 2D tensor to create a 3D tensor
            const inputTensor = inputTensor2D.expandDims(2);
